 ...
 *
 * Generated at: 2026-07-24T03:54:54.206Z
 * Agent count: 44
 */
...
export const bundledAgents: Record<string, any> = {
  ...
  "luna-agent": {
    ...
    "displayName": "Luna Agent",
    "spawnerPrompt": "A fast, affordable general-purpose agent powered by OpenAI GPT-5.6 Luna. Best for quick tasks, summaries, and lightweight problems where speed matters more than deep reasoning. This agent has no context on the conversation history so it cannot see files you have read or previous discussion. Instead, you must provide all the relevant context via the prompt or filePaths for this agent to work well.",
    "inputSchema": {
      "prompt": {
        "type": "string",
        "description": "The problem you are trying to solve"
  ...
  "gpt-5-agent": {
    ...
    "spawnerPrompt": "A general-purpose, deep-thinking agent powered by OpenAI GPT-5.6 Sol (flagship reasoning model). Use this to solve hard problems that need extended reasoning, especially as an alternative perspective to the Anthropic models. This agent has no context on the conversation history so it cannot see files you have read or previous discussion. Instead, you must provide all the relevant context via the prompt or filePaths for this agent to work well.",
    "inputSchema": {
      "prompt": {
        "type": "string",
        "description": "The problem you are trying to solve"
  ...
  "fable": {
    ...
    "spawnerPrompt": "A special deep-thinking agent powered by Claude Fable 5. Spawn this when you need extra thinking power on a hard problem: tricky architecture decisions, subtle bugs, or complex tradeoffs. Gather all the relevant context BEFORE spawning it, since it only thinks (it cannot read files or run tools).",
    "inputSchema": {
      "prompt": {
        "type": "string",
        "description": "The problem you are trying to solve, very briefly. No need to provide context, as the agent can see the entire conversation history."
      }
    },
    "outputMode": "last_message",
    "includeMessageHistory": true,
