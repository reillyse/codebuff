...
Create a new TypeScript file in `.agents/` directory. This example uses the [`researcher`](/publishers/codebuff/agents/researcher) as a spawnable subagent:
...
Writes REST and GraphQL docs with examples. Uses the [`researcher`](/publishers/codebuff/agents/researcher) for documentation lookup:
...
Builds and reviews database migrations. Uses the [`reviewer`](/publishers/codebuff/agents/reviewer) for checking migrations:
...
Use TypeScript generator functions with the `handleSteps` field to control execution. This example spawns [`thinker`](/publishers/codebuff/agents/thinker) and [`reviewer`](/publishers/codebuff/agents/reviewer) subagents:
